...
// use const Langivi\ImportantReminder\Utils\MIME_TYPES_MAP;
// require_once 'MimeType.php';

const MIME_TYPES_MAP = array(
...
function getFileMimeType(string $filename, bool $deepMode = true): string|bool 
{
    $type = false;
    ...
    if (!$deepMode) return $type; 

    if (!$type || in_array($type, UNCERTAIN_TYPES)) {
        $secondOpinion = exec('file -b --mime-type ' . escapeshellarg($filename));
        if ($secondOpinion) {
            $type = $secondOpinion;
            // var_dump('exec - ' , $type);
        }
    }
    // var_dump('mime_content_type - ' , mime_content_type($filename));
    // var_dump(function_exists('mime_content_type'));

    $ext = explode('.', $filename);
    $ext = strtolower(end($ext));
    if (array_key_exists($ext, MIME_TYPES_MAP)) {
        $type = MIME_TYPES_MAP[$ext];
    }
